...
(function (bodyScrollLock, themeAddresses, themeCurrency, Sqrl, themeImages, Flickity, FlickityFade, Rellax, AOS) {
  'use strict';
  ...
  class CartDrawer {
    ...
    estimateShippingCalculator() {
      Shopify.Cart.ShippingCalculator.show({
        ...
        moneyFormat: theme.settings.moneyWithCurrencyFormat,
      });
    }
...
// });
  ...
  class Slider {
    ...
    sliderSettle() {
      let animatedItems = this.slideshow.querySelectorAll(`.${classes.isSelected} ${selectors$7.dataAos}`);
      ...
      this.copyButtons.forEach((el) => {
        el.addEventListener('click', function (e) {
          ...
          if (this.hasAttribute(selectors$8.tooltip) && windowWidth > 749) {
            this.classList.add(classes$1.visible);
            this.dispatchEvent(new CustomEvent('tooltip:init', {bubbles: true}));

            setTimeout(() => {
              this.classList.remove(classes$1.visible);
              document.dispatchEvent(
    ...
    sidebarNav() {
      ...
      this.container.addEventListener('click', (e) => {
        ...
        if (!isMobile && checkLink && submenu) {
          submenu.parentElement.classList.toggle(classes$2.active);
          submenu.classList.toggle(classes$2.open);
          submenu.setAttribute('aria-expanded', submenu.classList.contains(classes$2.open));
          slideToggle(submenu);
      ...
      if (links.length) {
        links.forEach((element) => {
          ...
          if (href === location) {
            ...
            element.closest('li').classList.add(classes$2.active);
            if (!elementClosest) return;
            elementClosest.classList.add(classes$2.active);
            const submenu = elementClosest.querySelector(`.${classes$2.submenu}`);
  ...
  class Popout {
    constructor(popout) {
      ...
      this.productGrid = this.popoutList.closest(selectors$a.productGrid);
    ...
    popupToggleClick(evt) {
      ...
      if (this.productGrid) {
        const productGridItemImage = this.productGrid.querySelector(selectors$a.productGridImage);
...
// console.log('ff');
    ...
    updatePopout() {
      ...
      if (targetElement) {
        ...
        if (!targetElement.parentElement.nextSibling) {
          this.container.classList.add(classes$3.popoutAlternative);
        }
      } else {
        this.container.classList.add(classes$3.popoutAlternative);
      }
  ...
  const scrollTo = (elementTop) => {
    /* Sticky header check */
    const headerHeight =
      document.querySelector(selectors$b.headerSticky) && document.querySelector(selectors$b.headerHeight) ? document.querySelector(selectors$b.headerHeight).getBoundingClientRect().height : 0;
  ...
  class NewsletterCheckForResult {
    ...
    showMessage() {
      ...
      if (this.newsletter.id === this.formID) {
        ...
        const newsletterHeading = newsletter.parentElement.querySelector(selectors$c.newsletterHeading);
  ...
  const newsletterCheckForResultSection = {
    onLoad() {
      sections$5[this.id] = [];
      const newsletters = this.container.querySelectorAll(selectors$c.newsletterForm);
  ...
  class Tooltip {
    constructor(el, options = {}) {
      this.tooltip = el;
      if (!this.tooltip.hasAttribute(selectors$e.tooltip)) return;
      this.label = this.tooltip.getAttribute(selectors$e.tooltip);
    ...
    addPin(stopMouseEnter = false) {
      ...
      if (tooltipTarget && ((stopMouseEnter && !this.tooltip.hasAttribute(selectors$e.tooltipStopMouseEnter)) || !stopMouseEnter)) {
        ...
        tooltipTarget.classList.remove(classes$5.hiding);
        tooltipTarget.classList.add(classes$5.visible);
    ...
    removePin(event, stopMouseEnter = false, hideTransition = false) {
      const tooltipTarget = document.querySelector(`.${this.class}`);
      const tooltipVisible = tooltipTarget.classList.contains(classes$5.visible);

      if (tooltipTarget && ((stopMouseEnter && !this.tooltip.hasAttribute(selectors$e.tooltipStopMouseEnter)) || !stopMouseEnter)) {
        if (tooltipVisible && (hideTransition || event.detail.hideTransition)) {
          tooltipTarget.classList.add(classes$5.hiding);
        ...
        tooltipTarget.classList.remove(classes$5.visible);
  ...
  const tooltipSection = {
    onLoad() {
      sections$6[this.id] = [];
      const els = this.container.querySelectorAll(`[${selectors$e.tooltip}]`);
      els.forEach((el) => {
  ...
  class QuickAddProduct {
    ...
    onOptionChange(event) {
      ...
      const position = Number(optionHolder.getAttribute(selectors$h.elements.selectPosition));
  ...
  const quickAddProduct = {
    onLoad() {
      ...
      window.addEventListener("globoFilterRenderCompleted",function(e){ 
        self.container.querySelectorAll(`[${selectors$h.elements.productGridItem}]`).forEach((item) => {
          instances$1.push(new QuickAddProduct(item));
        });
      })
    },
    ...
    checkWidth() {
      ...
      if (this.frame.clientWidth - padding < this.comparitor.clientWidth || this.stopClone) {
        ...
        if (this.scale.childElementCount === 1) {
          ...
          const animationTimeFrame = (this.text.clientWidth / selectors$l.space) * selectors$l.moveTime;
    ...
    listenScroll(e) {
      if (e.detail.down) {
        this.unstickSimple();
         this.removeBlur();
//         if($(window).width() > 768){
//           if (!this.currentlyStuck && e.detail.position > 1000) {
//           //this.stickSimple();
//         }
//         }
//         else{
...
//         }
    ...
    stickSimple() {
      ...
      $('.announcement-sticky').addClass('fixed_announcement');
    ...
    unstickSimple() {
      if (!document.documentElement.hasAttribute('data-scroll-locked')) {
        ...
        $('.announcement-sticky').removeClass('fixed_announcement');
        this.wrapper.setAttribute(selectors$o.transparent, theme.settings.transparentHeader);
        if (this.animated) {
  ...
  class SearchPredictive {
    ...
    render(terms) {
      fetch(`/search/suggest.json?q=${encodeURIComponent(terms)}&resources[type]=product&resources[limit]=8&resources[options][z]=last`)
        .then(this.handleErrors)
        .then((response) => response.json())
        .then((response) => {
  ...
  class HeaderMobileSliderule {
    ...
    hideSliderule(close = false) {
      ...
      const hidedSelector = close ? `[${selectors$t.dataAnimates}].${classes$e.isHidden}` : `[${selectors$t.dataAnimates}="${newPosition}"].${classes$e.isHidden}`;
    ...
    checkPosition() {
      ...
      if (windowWidth < theme.sizes.small) {
        ...
        this.slides.forEach((slide, i) => {
          ...
          this.thumbs[i].classList.toggle(classes$f.isSelected, sliderPositionRight >= slidePositionRight && !nextSlideCheck);
        });
      }
    }
  ...
  class GlobalTabs {
    ...
    init() {
      ...
      const firstTabLink = ctx.querySelector(`${selectors$w.tabLink}-${ctx.hasAttribute(selectors$w.dataTabStartIndex) ? ctx.getAttribute(selectors$w.dataTabStartIndex) : 0}`);
      const firstTabContent = ctx.querySelector(`${selectors$w.tabContent}-${ctx.hasAttribute(selectors$w.dataTabStartIndex) ? ctx.getAttribute(selectors$w.dataTabStartIndex) : 0}`);

      if (firstTabContent) {
        firstTabContent.classList.add(classes$g.current);
      }

      if (firstTabLink) {
        firstTabLink.classList.add(classes$g.current);
      }
      ...
      if (tabsNavList.length) {
        tabsNavList.forEach((element) => {
          const tabId = parseInt(element.getAttribute(selectors$w.dataTab));
    ...
    tabChange(element, tab) {
      this.container.querySelector(`${selectors$w.tabsLi}.${classes$g.current}`).classList.remove(classes$g.current);
      const lastCurrentTab = this.container.querySelector(`${selectors$w.tabContent}.${classes$g.current}`);
      lastCurrentTab.classList.remove(classes$g.current);
    ...
    animateItems(tab, animated = true) {
      ...
      if (animatedItems.length) {
        animatedItems.forEach((animatedItem) => {
          animatedItem.classList.remove(classes$g.aosAnimate);

          if (animated) {
            animatedItem.classList.add(classes$g.aosNoTransition);

            setTimeout(() => {
              animatedItem.classList.remove(classes$g.aosNoTransition);
              animatedItem.classList.add(classes$g.aosAnimate);
            }, 100);
          }
        });
      }
    ...
    checkVisibleTabLinks() {
      ...
      const tabsNavListHided = this.container.querySelectorAll(`${selectors$w.tabLink}.${classes$g.hide}`);
    ...
    checkRecentTab() {
      ...
      if (tabLink) {
        tabLink.classList.remove(classes$g.hide);
        const tabLinkIdx = parseInt(tabLink.getAttribute(selectors$w.dataTab));
        const tabContent = this.container.querySelector(`${selectors$w.tabContent}[${selectors$w.dataTabIndex}="${tabLinkIdx}"]`);

        if (tabContent) {
          tabContent.classList.remove(classes$g.hide);
    ...
    hideRelatedTab() {
      ...
      const parentTabContent = relatedSection.closest(`${selectors$w.tabContent}.${classes$g.current}`);
      if (!parentTabContent) {
        return;
      }
      ...
      if (tabsNavList.length > parentTabContentIdx) {
        ...
        if (nextTabsNavLink) {
          tabsNavList[parentTabContentIdx].classList.add(classes$g.hide);
  ...
  const loaderHTML = `<div class="${classes$k.loader}"><div class="loader pswp__loader-line"><div class="loader-indeterminate"></div></div></div>`;
  ...
  class LoadPhotoswipe {
    ...
    init() {
      this.pswpElement.classList.add(classes$k.customOpen);
    ...
    initLoader() {
      if (this.pswpElement.classList.contains(classes$k.customLoader) && this.options !== '' && this.options.mainClass) {
    ...
    loadPopup() {
      ...
      if (this.pswpElement.classList.contains(classes$k.customLoader)) {
        this.pswpElement.classList.remove(classes$k.customLoader);
      }

      this.pswpElement.classList.remove(classes$k.customOpen);
    ...
    thumbsActions() {
      if (this.popupThumbsContainer && this.popupThumbsContainer.firstChild) {
        ...
        this.popupThumbs.forEach((element, i) => {
          element.addEventListener('click', (e) => {
            e.preventDefault();
            element.parentElement.querySelector(`.${classes$k.current}`).classList.remove(classes$k.current);
    ...
    onClose() {
      ...
      this.pswpElement.setAttribute(selectors$A.dataOptionClasses, '');
      const loaderElem = this.pswpElement.querySelector(`.${classes$k.loader}`);
      if (loaderElem) {
    ...
    setCurrentThumb() {
      const lastCurrentThumb = this.pswpElement.querySelector(`${selectors$A.popupThumbs} > .${classes$k.current}`);
      if (lastCurrentThumb) {
        lastCurrentThumb.classList.remove(classes$k.current);
      }

      if (!this.popupThumbs) return;
      const currentThumb = this.popupThumbs[this.popup.getCurrentIndex()];
      currentThumb.classList.add(classes$k.current);
      ...
      this.zoomWrappers.forEach((elementImage) => {
        ...
        if (this.zoomWrappers.length === counter) {
          const options = {
            ...
            focus: false,
            index: indexImage,
            mainClass: counter === 1 ? `${classes$l.popupClass} ${classes$l.popupClassNoThumbs}` : `${classes$l.popupClass}`,
  ...
  class Video {
    ...
    pauseOtherMedia(mediaId) {
      ...
      document.querySelector(`${selectors$C.productMediaWrapper}${mediaIdString}`).classList.remove(classes$m.mediaHidden);
  ...
  class Media {
    ...
    launch3d() {
      document.addEventListener('shopify_xr_launch', () => {
        const currentMedia = this.container.querySelector(`'${selectors$D.modelViewer}:not(.${classes$n.mediaHidden})'`);
  ...
  class PickupAvailability {
    ...
    fetchPickupAvailability(event) {
      ...
      if ((event && !event.detail.variant) || (event && event.detail.variant && !event.detail.variant.available)) {
        container.classList.add(classes$o.isHidden);
        return;
      }
      ...
      if (variantID) {
        fetch(`${window.theme.routes.root}variants/${variantID}/?section_id=api-pickup-availability`)
          ...
          .then((text) => {
            ...
            container.classList.remove(classes$o.isHidden);
    ...
    openDrawer() {
      if (this.drawer) {
        this.drawer.classList.add(classes$o.isOpen);
    ...
    closeDrawer() {
      if (this.drawer) {
        this.drawer.classList.remove(classes$o.isOpen);
  ...
  class ProductAddForm {
    constructor(section) {
      this.section = section;
      this.container = section.container;
      this.tallLayout = this.container.getAttribute(selectors$F.dataTallLayout) === 'true';
      ...
      this.planDecription = this.container.querySelector(selectors$F.subDescription);
      ...
      if (this.remainingWrapper) {
        ...
        this.remainingMaxInt = parseInt(remainingMaxWrap.getAttribute(selectors$F.remainingMaxAttr), 10);
      ...
      this.enableHistoryState = this.container.getAttribute(selectors$F.dataEnableHistoryState) === 'true';
    ...
    updateAddToCartState(formState) {
      ...
      if (priceWrapper.length && variant) {
        priceWrapper.forEach((element) => {
          element.classList.remove(classes$p.hide);
        });
      }
      ...
      if (formWrapper.length) {
        formWrapper.forEach((element) => {
          if (variant) {
            if (variant.available) {
              element.classList.remove(classes$p.variantSoldOut, classes$p.variantUnavailable);
            } else {
              element.classList.add(classes$p.variantSoldOut);
              element.classList.remove(classes$p.variantUnavailable);
            }
            ...
            element.classList.add(classes$p.variantUnavailable);
            element.classList.remove(classes$p.variantSoldOut);
          }
        });
      }
    }
    ...
    updateRemaining(formState) {
      ...
      if (variant && this.remainingWrapper && this.remainingJSON && this.remainingCount) {
        ...
        if (newQuantity && newQuantity <= this.remainingMaxInt && newQuantity > 0) {
          this.remainingWrapper.classList.remove(classes$p.remainingIn, classes$p.remainingOut, classes$p.remainingUnavailable);
          this.remainingWrapper.classList.add(classes$p.remainingLow);
          this.remainingCount.innerHTML = newQuantity;
        } else if (this.productState.soldOut) {
          this.remainingWrapper.classList.remove(classes$p.remainingLow, classes$p.remainingIn, classes$p.remainingUnavailable);
          this.remainingWrapper.classList.add(classes$p.remainingOut);
        } else if (this.productState.available) {
          this.remainingWrapper.classList.remove(classes$p.remainingLow, classes$p.remainingOut, classes$p.remainingUnavailable);
          this.remainingWrapper.classList.add(classes$p.remainingIn);
        }
      } else if (this.remainingWrapper) {
        this.remainingWrapper.classList.remove(classes$p.remainingIn, classes$p.remainingOut, classes$p.remainingLow);
        this.remainingWrapper.classList.add(classes$p.remainingUnavailable);
      }
    }
      ...
      toggles.forEach((toggle) => {
        toggle.addEventListener(
          ...
          function (e) {
            ...
            if (selected) {
              selected.classList.remove(classes$p.hide);
            ...
            groups.forEach((group) => {
              if (group !== selected) {
                group.classList.add(classes$p.hide);
    ...
    updateSaleText(formState) {
      if (this.productState.planSale) {
        ...
        this.priceOffWrap.classList.add(classes$p.hide);
      }
    }
    ...
    updateSaleTextStandard(formState) {
      ...
      this.priceOffWrap.classList.remove(classes$p.hide);
    }
    ...
    updateSubscriptionText(formState) {
      if (formState.plan && this.planDecription) {
        ...
        this.planDecription.classList.remove(classes$p.hide);
      } else if (this.planDecription) {
        this.planDecription.classList.add(classes$p.hide);
      }
    }
    ...
    updateProductPrices(formState) {
      ...
      const apoPriceOrigin = $('#apo-price-origin');
      ...
      priceWrappers.forEach((wrap) => {
        ...
        if (comparePriceEl) {
          if (this.productState.onSale || this.productState.planSale) {
            comparePriceEl.classList.remove(classes$p.hide);
            comparePriceText.classList.remove(classes$p.hide);
            productPriceEl.classList.add(classes$p.productPriceSale);
          } else {
            comparePriceEl.classList.add(classes$p.hide);
            comparePriceText.classList.add(classes$p.hide);
            productPriceEl.classList.remove(classes$p.productPriceSale);
          }
          comparePriceEl.innerHTML = 'RRP'+themeCurrency.formatMoney(comparePrice, theme.moneyFormat);
        }
		$('#apo-price-origin')[0].innerHTML = price === 0 ? window.theme.strings.free : themeCurrency.formatMoney(price, theme.moneyFormat);
    ...
    updateProductImage(evt) {
      ...
      if (variant) {
        ...
        if (variant.featured_media) {
          const newImg = this.container.querySelector(`${selectors$F.productImage}[${selectors$F.dataImageId}="${variant.featured_media.id}"]`);
  ...
  class GlobalAccordions {
    constructor(el) {
      ...
      this.accordion = this.container.querySelector(selectors$G.elements.accordion);
      this.accordionToggles = this.container.querySelectorAll(selectors$G.elements.accordionToggle);
      this.accordionTogglesLength = this.accordionToggles.length;
      this.accordionBody = this.container.querySelector(selectors$G.elements.accordionBody);
    ...
    accordionEvents() {
      this.accordionToggles.forEach((element) => {
        element.addEventListener(
          ...
          throttle((event) => {
            event.preventDefault();
            const targetAccordionBody = element.parentElement.querySelector(selectors$G.elements.accordionBody);
      ...
      if (this.accordion.getAttribute(selectors$G.elements.accordionExpandValue) === 'true') {
        this.accordionToggles[0].classList.add(selectors$G.classes.open);

        showElement(this.accordionToggles[0].parentElement.querySelector(selectors$G.elements.accordionBody));
      }
    }

    closeOtherAccordions(element, slide = true) {
      let otherElements = [...this.accordionToggles];
      const holder = this.container.closest(selectors$G.elements.accordionHolder);
      if (holder) {
        otherElements = [...holder.querySelectorAll(selectors$G.elements.accordionToggle)];
      }

      otherElements.filter((otherElement) => {
        const otherElementAccordionBody = otherElement.parentElement.querySelector(selectors$G.elements.accordionBody);
        if (otherElement !== element && otherElement.classList.contains(selectors$G.classes.open) && otherElementAccordionBody) {
    ...
    onAccordionOpen(element, body, slide = true) {
      element.classList.add(selectors$G.classes.open);
      slideDown(body);
    ...
    onAccordionClose(element, body, slide = true) {
      element.classList.remove(selectors$G.classes.open);
      if (slide) {
        slideUp(body);
      } else {
        hideElement(body);
    ...
    onAccordionToggle(element, body) {
      element.classList.toggle(selectors$G.classes.open);
      slideToggle(body);
    ...
    onBlockToggle(evt, blockSelect = true) {
      const targetAccordionToggle = this.container.querySelector(`${selectors$G.elements.accordionToggle}[${selectors$G.elements.accordionBlockValue}="${evt.detail.blockId}"]`);
      if (!targetAccordionToggle) return;
      const targetAccordionBody = targetAccordionToggle.parentElement.querySelector(selectors$G.elements.accordionBody);
  ...
  class ScrollToElement {
    constructor(section) {
      this.section = section;
      this.container = section.container;
      this.scrollToButtons = this.container.querySelectorAll(selectors$H.scrollToElement);
      this.scrollSpyElement = this.container.querySelector(selectors$H.scrollSpy);
    ...
    init() {
      this.scrollToButtons.forEach((element) => {
        element.addEventListener('click', (e) => {
          e.preventDefault();
          const target = document.querySelector(element.getAttribute(selectors$H.scrollToElementValue));

          if (!target) return;
          const accordionToggle = target.querySelector(selectors$H.accordionToggle);
          let timeoutFlag = false;
          const scrollToLinks = element.closest(selectors$H.scrollToLinks);

          if (scrollToLinks && !element.closest(selectors$H.scrollSpy)) {
            const selectedItems = scrollToLinks.querySelectorAll(`.${classes$q.selected}`);
            if (selectedItems.length) {
              selectedItems.forEach((selectedItem) => {
                selectedItem.classList.remove(classes$q.selected);
              });
            }
            element.classList.add(classes$q.selected);
          }

          // Open target accordion if they are inside it
          if (accordionToggle) {
            const accordionHolder = accordionToggle.closest(selectors$H.accordionHolder);

            if (!accordionToggle.classList.contains(classes$q.open) && accordionHolder && accordionHolder.querySelector(`${selectors$H.accordionToggle}.${classes$q.open}`)) {
              timeoutFlag = true;
            }

            if (!accordionToggle.classList.contains(classes$q.open)) {
    ...
    scrollToElement(element) {
      ...
      const tooltips = document.querySelectorAll(`${selectors$H.tooltip}:not([${selectors$H.tooltipStopMousenterValue}])`);
      if (tooltips.length) {
        tooltips.forEach((tooltip) => {
          tooltip.setAttribute(selectors$H.tooltipStopMousenterValue, '');

          setTimeout(() => {
            tooltip.removeAttribute(selectors$H.tooltipStopMousenterValue);
          }, 1000);
        });
      }
    }
    ...
    scrollSpy() {
      const buttons = this.scrollSpyElement.querySelectorAll(selectors$H.scrollToElement);
      if (!buttons) return;
      const headerHeight =
        document.querySelector(selectors$H.headerSticky) && document.querySelector(selectors$H.headerHeight) ? document.querySelector(selectors$H.headerHeight).getBoundingClientRect().height : 0;
      ...
      window.addEventListener('scroll', () => {
        ...
        buttons.forEach((element) => {
          ...
          if (target.offsetTop - headerHeight <= scrollPos) {
            this.scrollSpyElement.querySelector(`.${classes$q.selected}`).classList.remove(classes$q.selected);
            this.scrollSpyElement.querySelector(`[${selectors$H.scrollToElementValue}="#${target.id}"]`).classList.add(classes$q.selected);
          }
        });
      });
    }
  ...
  class Product {
    constructor(section) {
      ...
      this.tallLayout = this.container.getAttribute(selectors$I.dataTallLayout) === 'true';
      ...
      if (this.container.getAttribute(selectors$I.dataCartBar) === 'true') {
        this.initCartBar();
      }
    ...
    productPopup() {
      this.modalButton.forEach((button) => {
        button.addEventListener('click', (e) => {
          e.preventDefault();
          const modal = document.querySelector(`#${button.getAttribute(selectors$I.dataModalButton)}`);
          const modalScrollContainer = modal.querySelector(selectors$I.modalScrollContainer);

          if (window.getComputedStyle(modal).display !== 'none') {
            fadeOut(modal);
            this.formWrapper.classList.remove(classes$r.hasPopup);
            this.accessibility.removeTrapFocus();
          ...
          if (window.getComputedStyle(modal).display === 'none') {
            fadeIn(modal);
            this.formWrapper.classList.add(classes$r.hasPopup);
    ...
    calcUpsellButtonDemensions() {
      const upsellButtonText = this.upsellButton.querySelector(selectors$I.upsellButtonText);
      if (upsellButtonText) {
    ...
    stickyScrollCheck() {
      ...
      if (isDesktop) {
        ...
        if (productCopyHeight < productImagesHeight || productCopyHeight < window.innerHeight) {
          ...
          targetFormWrapper.classList.add(classes$r.sticky);
        } else {
          theme.variables.productPageSticky = false;
          targetFormWrapper.classList.remove(classes$r.sticky);
        }
      } else {
        targetFormWrapper.classList.remove(classes$r.sticky);
      }
    ...
    truncateText() {
      if (this.truncateElementHolder.classList.contains(classes$r.visible)) return;
      ...
      const truncateElementClass = this.truncateElement.getAttribute(selectors$I.toggleTruncateContentAttr);
      ...
      truncateAppendedElement.removeAttribute(selectors$I.toggleTruncateContentAttr);
      ...
      if (this.truncateElement.innerHTML !== truncateAppendedElement.innerHTML) {
        this.truncateElementHolder.classList.add(classes$r.expanded);
      } else {
        truncateAppendedElement.remove();
        this.truncateElementHolder.classList.remove(classes$r.expanded);
      }
    ...
    toggleTruncatedContent(holder) {
      ...
      if (toggleButton) {
        toggleButton.addEventListener('click', (e) => {
          e.preventDefault();
          holder.classList.remove(classes$r.expanded);
          holder.classList.add(classes$r.visible);
        });
      }
    }
      ...
      if (this.shareButton) {
        this.shareButton.addEventListener('click', function () {
          ...
          if (navigator.share && windowWidth <= 1024) {
            const shareTitle = this.hasAttribute(selectors$I.dataProductShareTitleValue) ? this.getAttribute(selectors$I.dataProductShareTitleValue) : window.location.hostname;
            const shareUrl = this.hasAttribute(selectors$I.dataProductShareValue) ? this.getAttribute(selectors$I.dataProductShareValue) : window.location.href;
            ...
            this.parentElement.classList.toggle(classes$r.expanded);
          }
        });
      }
    }
    ...
    initCartBar() {
      const cartBar = document.querySelector(selectors$I.cartBar);
      const cartBarBtn = cartBar.querySelector(selectors$I.productSubmitAdd);
      ...
      if (cartBarBtn) {
        cartBarBtn.addEventListener('click', (e) => {
          ...
          if (e.target.hasAttribute(selectors$I.cartBarAdd)) {
            if (theme.settings.cartDrawerEnabled) {
              e.target.classList.add(classes$r.loading);
              e.target.setAttribute('disabled', 'disabled');
              if (cartBarBtn && cartBarBtn.classList.contains(classes$r.loading)) {
                setTimeout(() => {
                cartBarBtn.classList.remove(classes$r.loading);
                cartBarBtn.removeAttribute('disabled');
              }, 1e3);
              }
              
            }
			this.form.querySelector(selectors$I.addToCart).click();
//             this.form.querySelector(selectors$I.addToCart).dispatchEvent(
//               new CustomEvent('click', {
//                 bubbles: true,
//                })
//             );
          } else if (e.target.hasAttribute(selectors$I.cartBarScroll)) {
            this.scrollToTop();
          }
        });

        if (cartBarBtn.hasAttribute(selectors$I.cartBarAdd)) {
    ...
    scrollTop() {
      ...
      const siteFooter = document.querySelector(selectors$I.siteFooterWrapper);
      ...
      if (this.form && this.cartBar) {
        ...
        this.cartBar.classList.toggle(classes$r.visible, checkPosition);
        siteFooter.classList.toggle(classes$r.siteFooterPush, checkPosition);
        siteFooter.style.marginBottom = siteFooter.classList.contains(classes$r.siteFooterPush) ? `${this.cartBar.offsetHeight}px` : '0';
      }
    }
  ...
  class Related {
    ...
    init() {
      const relatedSection = this.container.querySelector(selectors$J.dataRelatedSectionElem);
      ...
      const self = this;
      const productId = relatedSection.getAttribute(selectors$J.dataProductId);
      const limit = relatedSection.getAttribute(selectors$J.dataLimit);
      ...
      fetch(requestUrl)
        ...
        .then(function (data) {
          ...
          const inner = createdElement.querySelector(selectors$J.dataRelatedSectionElem);

          if (inner.querySelector(selectors$J.dataRelatedProduct)) {
            const innerHtml = inner.innerHTML;
            hideElement(relatedSection);
            relatedSection.innerHTML = innerHtml;
            slideDown(relatedSection);

            const relatedProducts = relatedSection.querySelectorAll(selectors$J.dataRelatedProduct);
            ...
            if (relatedProducts.length > 4 && relatedSection.querySelector(selectors$J.slider)) {
    ...
    recent() {
      const recentlyViewed = this.container.querySelector(selectors$J.recentlyViewed);
      const howManyToshow = recentlyViewed ? parseInt(recentlyViewed.getAttribute(selectors$J.dataLimit)) : 4;
      ...
      Shopify.Products.showRecentlyViewed({
        ...
        onComplete: (wrapper, section) => {
          ...
          const recentlyViewedHolder = container.querySelector(selectors$J.recentlyViewed);
          const recentlyViewedWrapper = container.querySelector(selectors$J.recentlyViewedWrapper);
          const recentProducts = wrapper.querySelectorAll(selectors$J.dataProductItem);
          const minimumNumberProducts = recentlyViewedHolder.hasAttribute(selectors$J.dataMinimum) ? parseInt(recentlyViewedHolder.getAttribute(selectors$J.dataMinimum)) : 4;
          ...
          if (checkRecentInRelated || checkRecentOutsideRelated) {
            if (checkRecentOutsideRelated) {
              recentlyViewedWrapper.classList.remove(classes$s.isHidden);
            }
            ...
            if (recentProducts.length > 4 && recentlyViewedHolder.querySelector(selectors$J.slider)) {
  ...
  const relatedSection = {
    onLoad() {
      ...
      this.container.querySelectorAll(selectors$J.dataRelatedProduct).forEach((item) => {
        new QuickAddProduct(item);
      });
    },
  };
  ...
  class BlockScroll {
    ...
    onBlockSelect(evt) {
      const scrollElement = this.container.querySelector(selectors$K.scrollElement);
      if (scrollElement && !scrollElement.classList.contains(selectors$K.flickityEnabled)) {
  ...
  class LogoList {
    constructor(section) {
      ...
      this.slideshowText = this.container.querySelector(selectors$L.text);
    ...
    init() {
      if (this.slideshowText) {
        ...
        const textSlides = this.slideshowText.querySelectorAll(selectors$L.slide);
      ...
      this.logoSlides.forEach((element) => {
        element.addEventListener('click', (e) => {
          const index = parseInt(e.currentTarget.getAttribute(selectors$L.slideIndex));
          const hasSlider = this.slideshowNav.classList.contains(selectors$L.flickityEnabled);
          ...
          if (hasSlider) {
            ...
            if (!this.slideshowNav.classList.contains(classes$t.isSelected)) {
              this.flktyNav.playPlayer();
            }
          } else {
            const selectedSlide = this.slideshowNav.querySelector(`.${classes$t.isSelected}`);
            if (selectedSlide) {
              selectedSlide.classList.remove(classes$t.isSelected);
            }
            e.currentTarget.classList.add(classes$t.isSelected);
          }
        });
      });
    ...
    onUnload() {
      if (!this.slideshowNav) return;
      const sliderInitialized = this.slideshowNav.classList.contains(selectors$L.flickityEnabled);
    ...
    onBlockSelect(evt) {
      if (!this.slideshowNav) return;
      const slide = this.slideshowNav.querySelector(`[${selectors$L.slideData}="${evt.detail.blockId}"]`);
      const slideIndex = parseInt(slide.getAttribute(selectors$L.slideIndex));

      if (this.slideshowNav.classList.contains(selectors$L.flickityEnabled)) {
        this.flktyNav.select(slideIndex);
        this.flktyNav.stopPlayer();
        this.slideshowNav.classList.add(classes$t.isSelected);
      } else {
    ...
    onBlockDeselect() {
      if (this.slideshowNav && this.slideshowNav.classList.contains(selectors$L.flickityEnabled)) {
        this.flktyNav.playPlayer();
        this.slideshowNav.classList.remove(classes$t.isSelected);
      }
    }

    setSlideshowNavState() {
      const slidesCount = this.slideshowNav.querySelectorAll(selectors$L.slide).length;
      ...
      if (slidesWidth > windowWidth) {
        if (!sliderInitialized) {
          ...
          this.logoSlides[0].classList.add(classes$t.isSelected);
  ...
  class VideoPlay {
    constructor(section, selector = selectors$M.videoPlay, selectorValue = selectors$M.videoPlayValue) {
  ...
  class CompareImages {
    constructor(section) {
      ...
      if (this.imagesHolder) {
        this.imageHolder = this.imagesHolder.querySelector(selectors$N.imageHolder);
        this.imageElement = this.imagesHolder.querySelector(selectors$N.imageElement);
        this.imagesButton = this.imagesHolder.querySelector(selectors$N.imagesButton);
        this.startPosition = this.imagesHolder.hasAttribute(selectors$N.dataStartPosition) ? this.imagesHolder.getAttribute(selectors$N.dataStartPosition) : 0;
        this.startX = 0;
        this.x = 0;
  ...
  const compareImages = {
    onLoad() {
      sections$o[this.id] = [];
      const els = this.container.querySelectorAll(selectors$N.imagesHolder);
      els.forEach((el) => {
        sections$o[this.id].push(new CompareImages(el));
      });
    },
    onUnload() {
  ...
  class DelayShow {
    constructor(holder, element, callback = null) {
      this.element = element;
      this.delay = holder.getAttribute(selectors$O.delayAttribite);
  ...
  class TargetReferrer {
    constructor(el) {
      ...
      if (!this.el.hasAttribute(selectors$O.dataTargetReferrer)) {
        return false;
      }
    ...
    init() {
      if (this.locationPath.indexOf(this.el.getAttribute(selectors$O.dataTargetReferrer)) === -1 && !window.Shopify.designMode) {
  ...
  class LargePopup {
    constructor(el) {
      this.popup = el;
      this.modal = this.popup.querySelector(selectors$O.largePromoInner);
      this.modalBody = this.popup.querySelector(selectors$O.modalBody);
      this.close = this.popup.querySelector(selectors$O.close);
      this.underlay = this.popup.querySelector(selectors$O.modalUnderlay);
      this.form = this.popup.querySelector(selectors$O.newsletterForm);
      this.cookie = new PopupCookie(this.popup.getAttribute(selectors$O.cookieNameAttribute), 'user_has_closed');
    ...
    init() {
      ...
      if (!cookieExists || window.Shopify.designMode) {
        ...
        if (this.form && this.form.classList.contains(classes$u.success)) {
    ...
    onBlockSelect(evt) {
      if (this.popup.contains(evt.target)) {
        fadeIn(this.modal, null, () => this.scrollLock());
        this.popup.classList.add(classes$u.selected);
        this.popup.parentNode.classList.add(classes$u.hasBlockSelected);
      }
    }
    ...
    onBlockDeselect(evt) {
      if (this.popup.contains(evt.target)) {
        fadeOut(this.modal);
        this.scrollUnlock();
        this.popup.classList.remove(classes$u.selected);
        this.popup.parentNode.classList.remove(classes$u.hasBlockSelected);
      }
    }
  }
  ...
  class Tracking {
    constructor(el) {
      this.popup = el;
      this.modal = document.querySelector(selectors$O.tracking);
      this.acceptButton = this.modal.querySelector(selectors$O.trackingAccept);
  ...
  class NewsletterPopup {
    constructor(el) {
      this.popup = el;
      this.holder = this.popup.querySelector(selectors$O.newsletterPopupHolder);
      this.close = this.popup.querySelector(selectors$O.newsletterClose);
      this.heading = this.popup.querySelector(selectors$O.newsletterHeading);
      this.newsletterField = this.popup.querySelector(selectors$O.newsletterField);
      this.cookie = new PopupCookie(this.popup.getAttribute(selectors$O.cookieNameAttribute), 'newsletter_is_closed');
      this.form = this.popup.querySelector(selectors$O.newsletterForm);
      this.isTargeted = new TargetReferrer(this.popup);
    ...
    init() {
      ...
      if (!cookieExists || window.Shopify.designMode) {
        ...
        if (this.form.classList.contains(classes$u.success)) {
          this.checkForSuccess();
        }
      }
    }
    ...
    showForm() {
      this.heading.addEventListener('click', (event) => {
        ...
        this.heading.classList.add(classes$u.hide);
    ...
    inputField() {
      ...
      this.newsletterField.addEventListener('focusout', () => {
        setTimeout(() => {
          this.holder.classList.remove(classes$u.hasValue);
        }, 2000);
      });
    }
  ...
  const popupSection = {
    onLoad() {
      ...
      newsletters.forEach((el) => {
        sections$p[this.id].push(new LargePopup(el));
      });

      const tracking = this.container.querySelectorAll(selectors$O.tracking);
      tracking.forEach((el) => {
        sections$p[this.id].push(new Tracking(el));
      });

      const newsletterPopup = this.container.querySelectorAll(selectors$O.newsletterPopup);
      newsletterPopup.forEach((el) => {
        sections$p[this.id].push(new NewsletterPopup(el));
      });

      const promoPopup = this.container.querySelectorAll(selectors$O.promoPopup);
      promoPopup.forEach((el) => {
        sections$p[this.id].push(new PromoText(el));
      });
    },
    onBlockSelect(evt) {
  ...
  class Password {
    constructor(section) {
      this.container = section.container;
      this.loginToggle = this.container.querySelector(selectors$P.loginToggle);
      this.newsletterToggle = this.container.querySelector(selectors$P.newsletterToggle);
      this.login = this.container.querySelector(selectors$P.login);
      this.signup = this.container.querySelector(selectors$P.signup);
      this.errors = this.container.querySelector(selectors$P.errors);
      this.contactErrors = this.container.querySelector(selectors$P.contactErrors);
      this.loginErrors = this.container.querySelector(selectors$P.loginErrors);
      this.init();
    }
...
function showCartcollection(countItems) {
  if(countItems > 0) {
//     $('.cart-drawer-recommended').slick({  
//     	speed: 900,
//     	arrows: true,
...
//    		nextArrow: '<button class="cstm-slick-next"><i class="fa fa-angle-right"></i></button>'
//      });
    $('.cart-dropdown__items_collection').show();
  }
  else {
    $('.cart-dropdown__items_collection').hide();
  }
}